<x-dashboard.layouts.base-dashboard title="ubah dokumen kinerja">
    {{-- todo css --}}
    <x-slot:hadeOptional>
    ...
    <x-slot:content>
        ...
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">ubah Dokumen Kinerja</h1>
        </div>

        {{-- ? DataTales Example  --}}
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary">Ubah Dokumen Kinerja</h6>
            </div>
            <div class="card-body">
                <form action="{{ route('dokument-kinerja.update', $dokumentKinerja) }}" method="POST">
                    @csrf
                    @method('patch')
                    {{-- ? head surat --}}
                    <x-dashboard.subComponents.input-textarea
                        ...
                        >
                        {{ old('head_dokumen', $dokumentKinerja->head_dokument) }}
                    </x-dashboard.subComponents.input-textarea>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 border-right">
                            ...
                            <x-dashboard.subComponents.input
                                ...
                                type="text"
                                valueData="{{ $dokumentKinerja->userPertama->biodata->nama_lengkap }}"
                                disable="true"
                            />
                            {{-- ? nip --}}
                            <x-dashboard.subComponents.input
                                ...
                                type="text"
                                valueData="{{ $dokumentKinerja->userPertama->nip }}"
                                disable="true"
                            />
                            {{-- ? pangkat --}}
                            <x-dashboard.subComponents.input
                                ...
                                type="text"
                                valueData="{{ $dokumentKinerja->userPertama->biodata->pangkat_golongan }}"
                                disable="true"
                            />
                            {{-- ? bidang --}}
                            <x-dashboard.subComponents.input
                                ...
                                type="text"
                                valueData="{{ $dokumentKinerja->userPertama->biodata->bidang }}"
                                disable="true"
                            />
                            {{-- ? jabatan --}}
                            <x-dashboard.subComponents.input
                                ...
                                type="text"
                                valueData="{{ $dokumentKinerja->userPertama->biodata->jabatan->nama_jabatan }}"
                                disable="true"
                            />
                        </div>
                        {{-- ? Pihak kedua --}}
                        <div class="col-md-6 border-right">
                            ...
                            <x-dashboard.subComponents.input-select name="user_id_pihak_kedua" label="Nama Lengkap">
                                ...
                                @foreach ($user as $u)
                                    <option value="{{ $u->id }}" {{ old('user_id_pihak_kedua', $dokumentKinerja->user_id_pihak_kedua) == $u->id ? 'selected' : '' }}>
                            ...
                            <x-dashboard.subComponents.input
                                ...
                                type="text"
                                valueData="{{ $dokumentKinerja->userKedua->nip }}"
                                disable="true"
                            />
                            {{-- ? pangkat --}}
                            <x-dashboard.subComponents.input
                                ...
                                type="text"
                                valueData="{{ $dokumentKinerja->userKedua->biodata->pangkat_golongan }}"
                                disable="true"
                            />
                            {{-- ? bidang --}}
                            <x-dashboard.subComponents.input
                                ...
                                type="text"
                                valueData="{{ $dokumentKinerja->userKedua->biodata->bidang }}"
                                disable="true"
                            />
                            {{-- ? jabatan --}}
                            <x-dashboard.subComponents.input
                                ...
                                type="text"
                                valueData="{{ $dokumentKinerja->userKedua->biodata->jabatan->nama_jabatan }}"
                                disable="true"
                            />
                        </div>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3">
                            <x-dashboard.subComponents.input-select name="jenis_kinerja" label="Jenis Kinerja">
                                ...
                                @foreach ($jenis_kinerja as $jk)
                                    <option value="{{ $jk }}" {{ old('jenis_kinerja', $dokumentKinerja->jenis_kinerja) == $jk ? 'selected' : '' }}>
                                        {{ $jk }}
                                    </option>
                                @endforeach
                            </x-dashboard.subComponents.input-select>
                        ...
                        <div class="col-md-2">
                            <x-dashboard.subComponents.input-select name="tahun" label="Tahun">
                                <option value="pilih" selected disabled>Pilih</option>
                                @foreach ($tahun as $t)
                                    <option value="{{ $t }}" {{ old('tahun', $dokumentKinerja->tahun) == $t ? 'selected' : '' }}>
                                        {{ $t }}
                                    </option>
                                @endforeach
                            </x-dashboard.subComponents.input-select>
